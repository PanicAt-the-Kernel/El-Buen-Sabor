...
import {
  getCategoriasHijaIdSucursal,
  getCategoriasPadreIdSucursal,
} from "../../../../servicios/CategoriaService";
import Imagen from "../../../../entidades/Imagen";
import DeleteIcon from "@mui/icons-material/Delete";
import { localSession } from "../../../../servicios/localSession";
import StockInsumoSucursal from "../../../../entidades/StockInsumoSucursal";
...
function AgregarInsumoModal({
  ...
  if (insumo.stocksInsumo[0] == undefined) {
    let insu = insumo;
    let stock = new StockInsumoSucursal();
    stock.sucursal = localSession.getSucursal("sucursal");
    insu.stocksInsumo.push(stock);
    setInsumo(insu);
  }
  const handleSubmit = () => {
    ...
    if (
      insumo.stocksInsumo[0].stockActual < insumo.stocksInsumo[0].stockMinimo ||
      insumo.stocksInsumo[0].stockActual > insumo.stocksInsumo[0].stockMaximo
    ) {
    ...
    const selectedUMedida = unidadesMedida?.find(
      (um) => um.id === unidadMedidaL
    );

    if (!selectedCategoria || !selectedUMedida) {
      console.error(
        "La categoría o unidad de medida seleccionados son inválidos."
      );
      return;
    }
        ...
        >
          <Stack spacing={2}>
            <TextField
              ...
              onChange={(e) =>
                setInsumo({ ...insumo, denominacion: e.target.value })
              }
              inputProps={{
                maxLength: 30,
              }}
            ...
            <TextField
              ...
              onChange={(e) =>
                setInsumo({ ...insumo, precioVenta: parseInt(e.target.value) })
              }
              inputProps={{
                min: 0,
                step: 0.01,
            ...
            <TextField
              ...
              onChange={(e) =>
                setInsumo({ ...insumo, precioCompra: parseInt(e.target.value) })
              }
              inputProps={{
                min: 0,
                step: 0.01,
            ...
            <TextField
              ...
              onChange={(e) => {
                ...
                const newStockInsumo = [...insumo.stocksInsumo];
                newStockInsumo[0] = {
                  ...newStockInsumo[0],
                  stockMinimo: valor,
                };
                setInsumo({ ...insumo, stocksInsumo: newStockInsumo });
              }}
              inputProps={{
                min: 0,
                step: 0.01,
            ...
            <TextField
              ...
              type="number"
              inputProps={{
                min: insumo.stocksInsumo[0].stockMinimo,
                max: insumo.stocksInsumo[0].stockMaximo,
              }}
            ...
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  onChange={(e) =>
                    setInsumo({ ...insumo, esParaElaborar: e.target.checked })
                  }
                  name="esParaElaborar"
                  color="primary"
                />

...
function EditarSubCategoriasModal({
  iCategoria,
}: EditarSubCategoriasModalProps) {
  ...
  useEffect(() => {
    if (categoriasSuc) {
      const subCatAux = categoriasSuc.filter(
        (categoria: Categoria) =>
          categoria.id !== iCategoria.id &&
          !iCategoria.subCategorias?.some(
            (catSuc: Categoria) => catSuc.id === categoria.id
          )
      );
              ...
              <ListItem
                ...
                secondaryAction={
                  <Stack direction="row" spacing={1}>
                    <Button
                      onClick={() => handleOpenNombre(item)}
                      endIcon={<Edit />}
                    />
                    <IconButton
                      edge="end"
                      aria-label="Eliminar"
                      onClick={() => {handleEliminarCategoria(item)}}
                    >
                      <Remove />
                    </IconButton>
                  </Stack>
                ...
                secondaryAction={
                  <Stack direction="row" spacing={1}>
                    <IconButton
                      edge="end"
                      aria-label="Eliminar"
                      onClick={() => {handleAgregarCategoria(item);}}
                    >
                      <Add />
                    </IconButton>
                  </Stack>

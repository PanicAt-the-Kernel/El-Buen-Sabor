...
export function getAllClientes():SWRResponse<Cliente[],any,any>{
  return useSWR<Cliente[]>("https://traza-ending.onrender.com/cliente",fetcher);
}
...
export function getClienteEmail(
  ...
  if (clienteEmail != undefined) {
    return useSWR<Cliente>(
      `https://traza-ending.onrender.com/cliente/${clienteEmail}`,
      fetcher
    );
  }
...
export function getClienteId(
  ...
  return useSWR<Cliente>(
    `https://traza-ending.onrender.com/cliente/${idCliente}`,
    fetcher
  );
}
...
export async function editCliente(cliente: Cliente) {
  ...
  try {
    let response = await fetch(
      `https://traza-ending.onrender.com/cliente/${cliente.userName}`,
      options
    );
    if (response.ok) {
